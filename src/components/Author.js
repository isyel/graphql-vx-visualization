		...
		<div className="bg-gray-200 mx-2 my-4 p-2 rounded-lg grid grid-cols-4">
			<div className="col-span-3 flex justify-items-start items-center">
				...
				<div className="justify-start flex flex-col justify-items-start pl-3">
					...
					<p>
						Latest Post: <span className="text-gray-700">{posts[0].title}</span>
					</p>
				</div>
			</div>
			<div className="col-span-1 items-center content-center place-items-center flex">
				<div className="pr-6">
					<span className="text-xl">{posts.length}</span> posts
				</div>
				<Link
					to={`/profile/${author.id}`}
					className="py-2 px-3 rounded-md bg-gray-900 text-white">
					See Profile
				</Link>
			</div>
		</div>
